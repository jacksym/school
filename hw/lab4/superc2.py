...
class voltemp:
    ...
    def plot(self, ax, label):
        ...
        t10 = self.temps[t10i]
        ax.axvline(t10, linestyle = '--', linewidth=0.75,color = 'green')

        t50i = np.argmin(abs(self.amps - 0.5*r1))
        t50 = self.temps[t50i]
        ax.axvline(t50, linestyle = '--', linewidth=0.75,color='yellow')

        t90i = np.argmin(abs(self.amps - 0.9*r1))
        t90 = self.temps[t90i]
        ax.axvline(t90, linestyle = '--',linewidth=0.75,color='red')
...
ticks = [0,40]
ax.set_yticks(ticks)
for tick in ticks: ax.axhline(tick,color='grey',linewidth=0.75)
ax.set_xlim(9.8,10.2)
ax.set_title('Ramp (5 V Lock-In Output)')
...
plt.table(cellText=rows,cellLoc='left')
plt.legend()
fig.savefig('./images/superchl.png')
plt.show()
